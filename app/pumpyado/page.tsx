...
export default function FlappyAdo() {
  ...
  return (
    <>
      ...
      {/* Instructions */}
      {gameState.isPlaying && scoresLoaded && (
        <div className="absolute top-4 left-4 text-white text-lg font-bold" style={{ 
          fontFamily: '"Comic Neue", cursive',
          textShadow: '2px 2px 0px #000, -2px -2px 0px #000, 2px -2px 0px #000, -2px 2px 0px #000'
        }}>
          <p>Score: {Math.floor(gameState.score / 2)}</p>
          <p>Global Best: {Math.floor(gameState.highScore / 2)}</p>
          {hasPlayedBefore && personalBest > 0 && (
            <p>Your Best: {Math.floor(personalBest / 2)}</p>
          )}
          {hasPlayedBefore && playerRank > 0 && (
            <p>Your Rank: #{playerRank}</p>
          )}
          <p>Click Speed: {clickSpeed.toFixed(1)}/s</p>
        </div>
      )}
